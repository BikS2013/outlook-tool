...
## Removed: Agent Subcommand (Plan 003) and Agent Interactive TUI (Plan 004)

The original §s for the LangGraph ReAct agent (system architecture,
provider registry, tool adapters, ReAct loop, JSON envelope, error
mapping, system prompt, parallel implementation units, ADRs, NFRs) and
the Agent Interactive TUI (architecture, module catalogue, public
TypeScript interfaces, streaming event loop, keyboard state machine,
file formats, slash-command dispatcher, `/model` grammar, error handling,
test seams, acceptance criteria, parallel implementation units, design
decisions) were removed in **v3.0.0 (2026-04-30)** along with the
implementation. See `CHANGELOG.md` 3.0.0 for the breaking-change record
and the rationale.
