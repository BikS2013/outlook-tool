...
describe('list-mail --from / --to', () => {
  it('(fast path) builds $filter=ReceivedDateTime ge X and lt Y when both bounds are set', async () => {
    ...
    expect(path).toBe('/api/v2.0/me/MailFolders/Inbox/messages');
  ...
  it('(fast path) builds only lower bound when only --from is set', async () => {
    ...
    const [, query] = (client.get as ReturnType<typeof vi.fn>).mock.calls[0] as [string, Record<string, string>];
    expect(query.$filter).toMatch(/^ReceivedDateTime ge \S+$/);
    expect(query.$filter).not.toContain(' and ');
  });
  ...
  it('accepts "now - Nd" keyword for --from', async () => {
    ...
    const [, query] = (client.get as ReturnType<typeof vi.fn>).mock.calls[0] as [string, Record<string, string>];
    // The resolved ISO should parse back cleanly
    const match = query.$filter.match(/ge (\S+)/);
