...
export interface OutlookClient {
   ...
   * `$select` / `$orderby` / `$top` / `$filter` options as the existing
   * list-mail code path. `folderId` may be a raw id or a well-known alias.
   */
...
export function createOutlookClient(opts: CreateClientOptions): OutlookClient {
  ...
  async function countMessagesInFolder(
    ...
    const query: Record<string, QueryValue> = {
      $count: 'true',
      // Keep the wire payload minimal. We only need the envelope's
      // @odata.count; the single returned message is discarded.
      $top: '1',
      $select: 'Id',
    };
    if (typeof opts.filter === 'string' && opts.filter.length > 0) {
    ...
    try {
      ...
      return { count: Array.isArray(resp.value) ? resp.value.length : 0, exact: false };
    } catch (err) {
      throw mapHttpToCliError(err);
    }
  }
  ...
  async function listMessagesByConversation(
    ...
    // OData string literals use single quotes; any single quote inside the
    // value must be doubled. Outlook ConversationIds are base64-safe so this
    // is defensive rather than typical.
