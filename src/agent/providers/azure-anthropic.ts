...
//   AZURE_AI_INFERENCE_KEY       — required (shared with azure-deepseek)
//   AZURE_AI_INFERENCE_ENDPOINT  — required (shared with azure-deepseek)
//   AZURE_ANTHROPIC_MODEL        — optional (user-convenience mirror of
...
// cross-check: if `AZURE_ANTHROPIC_MODEL` is also set and disagrees with
// `cfg.model`, throw `UsageError` so the user is forced to decide. We never
// read `AZURE_ANTHROPIC_MODEL` as an alternative source of the model — that
...
const ENV_MODEL = 'AZURE_ANTHROPIC_MODEL';
...
export const createAzureAnthropicModel: ProviderFactory = (
  ...
  // Cross-check AZURE_ANTHROPIC_MODEL against cfg.model. If both are set AND
  // disagree → UsageError; if only the env is set, cfg.model still wins
  // (loadAgentConfig already required OUTLOOK_AGENT_MODEL / --model).
