...
//
// Env-var contract (design §4 + §5.6):
//   OUTLOOK_AGENT_AZURE_AI_INFERENCE_KEY       (shared with azure-anthropic)
//   OUTLOOK_AGENT_AZURE_AI_INFERENCE_ENDPOINT  (shared with azure-anthropic)
//   OUTLOOK_AGENT_AZURE_DEEPSEEK_MODEL         (user-convenience mirror of
...
// cross-check: if `OUTLOOK_AGENT_AZURE_DEEPSEEK_MODEL` is also set and
// disagrees with `cfg.model`, throw `UsageError` so the user is forced to
// decide. We never read `OUTLOOK_AGENT_AZURE_DEEPSEEK_MODEL` as an
...
const ENV_MODEL = 'OUTLOOK_AGENT_AZURE_DEEPSEEK_MODEL';
 ...
 * OUTLOOK_AGENT_AZURE_DEEPSEEK_MODEL` and a descriptive `detail` when the
 * variant is known-broken.
 */
 ...
 * is the source of truth. `OUTLOOK_AGENT_AZURE_DEEPSEEK_MODEL` may also be
 * set; if both are set AND disagree → UsageError.
 */
